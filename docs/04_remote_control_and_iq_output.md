...
| `l` / `L <level> <value>` | get / set a hamlib "level" — `AF` (audio/volume, 0.0-1.0), wired to `rx_audio.c`'s `rx_audio_get_volume()`/`rx_audio_set_volume()`; `STRENGTH` (S-meter, read-only); `CWPITCH` (stage 3's center pitch in Hz, which also moves the stage-2 BFO so the heard tone follows); and the `MICGAIN` and `CWWIDTH` extensions (`sound.c`'s mic_tx_gain, and stage 3's width in Hz). `CWPITCH`/`CWWIDTH` snap to the nearest entry in stage 3's pre-designed filter bank (`src/narrow_filter_bank.h`), so a client should trust the `l` readback over its own request. Every other hamlib level (`RF`, `SQL`, preamp, ...) gets an error reply, same as an unknown command |
