# TX: the original direct-to-DAC CW scheme (history)

Status: historical - replaced by `tx_pipeline.c` in `ARCHITECTURE.md`
§10 step 5. This is the previous `docs/03_tx_processing_pipeline.md`,
kept below as it was, with only its links adjusted for the new location. The current TX chain is
...
Note: [`dsp_design_notes/tx_power_calibration.md`](tx_power_calibration.md)
...
[`dsp_design_notes/antialias_filter_design.md`](antialias_filter_design.md))
...
- **`TX_SAMPLE_CLAMP`** — hard-limits the PCM sample so it can't wrap
  ...
  [`dsp_design_notes/tx_power_calibration.md`](tx_power_calibration.md)
...
- **`sound_set_rx_capture()`** (`sound.c`) — mutes the WM8731 "Capture"
  ...
  [`dsp_design_notes/rx_gain_and_level_calibration.md`](rx_gain_and_level_calibration.md))
